...
test("process testing normalizer", () => {
  ...
  assert.strictEqual(
    p.process('/*%= normalize("María _  José") %*/'),
    "mariaJose"
  );
...
test("process testing camelToSnake", () => {
  ...
  assert.strictEqual(
    p.process('/*%= camelToSnake("asdfAsdf") %*/'),
    "asdf_asdf"
  );
  assert.strictEqual(
    p.process('/*%= camelToSnake("asdfAsdfAsdf") %*/'),
    "asdf_asdf_asdf"
  );
  assert.strictEqual(
    p.process('/*%= camelToSnake("asdfAsdfAsdfAsdf") %*/'),
    "asdf_asdf_asdf_asdf"
  );
  assert.strictEqual(
    p.process('/*%= camelToSnake("asdf_asdf_asdf_asdf") %*/'),
    "asdf_asdf_asdf_asdf"
  );
  assert.strictEqual(p.process('/*%= camelToSnake("España") %*/'), "españa");
  assert.strictEqual(p.process('/*%= camelToSnake("María") %*/'), "maría");
  assert.strictEqual(
    p.process('/*%= camelToSnake("MaríaJosé") %*/'),
    "maría_josé"
  );
});
...
test("process different text with different custom delimiters", () => {
  ...
  assert.strictEqual(
    askGreetProc.process(f("template-engine/custom-delimiter.html"), "html"),
    f("template-engine/custom-delimiter-greet.html")
  );
  assert.strictEqual(
    notAskByeProc.process(f("template-engine/custom-delimiter.html"), "html"),
...
test("simple data", () => {
  ...
  assert.strictEqual(p.process("/*%= data.property %*/"), "asdf");
});
