...
test("Get features and parameters of an annotated project", async () => {
  ...
  assert.deepEqual(report.filesByFeature("featureA"), [
    "index.html",
    "main.js",
  ]);

  assert.deepEqual(report.filesByFeature("featureB"), ["index.html"]);

  assert.deepEqual(report.filesByData("aValue"), ["main.js"]);
});
...
test("Compare feature model vs analysed code results", async () => {
  ...
  assert.deepEqual(report.checkAnnotatedFeaturesConsistency(), {
    surplus: {
